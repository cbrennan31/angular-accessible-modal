...
@Component({
  ...
  animations: [
    trigger('entranceTrigger', [
      transition(':enter', [
        style({ opacity: "{{ initialOpacity }}", transform: "{{ initialYPosition }}" }),
        animate("{{ entranceDuration }}", style({ opacity: 1, transform: 'translateY(0)' }))
      ]),
    ]),
    trigger('exitTrigger', [
      transition(':leave', [
        animate(
          "{{ exitDuration }}", 
          style({ opacity: "{{ exitOpacity }}", transform: "{{ exitYPosition }}" })
        )
      ])
    ])
  ]
